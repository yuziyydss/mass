...
<tr><td>GET</td><td><code>/api/next-run</code></td><td>-</td><td>下次调度</td></tr>
                    ...
                    self._send_json({"date": date, "codes": codes, "count": len(codes)})
                ...
                elif path == "/api/factor-score":
                    self._send_json(factor_analysis.factor_effectiveness_score(config.db_path, factor_col=qs.get("factor", ["mass_zscore"])[0], forward_days=int(qs.get("days", ["5"])[0])))
                    ...
                    for c in comps_str.split(","):
                        ...
                        if len(parts) >= 1 and parts[0].strip():
                            comps.append({"name": parts[0].strip(), "weight": float(parts[1]) if len(parts)>1 else 1.0, "sign": float(parts[2]) if len(parts)>2 else 1.0})
                    self._send_json(factor_analysis.portfolio_concentration_check(config.db_path, comps, int(qs.get("n", ["30"])[0]), float(qs.get("max", ["30"])[0])))
                elif path == "/api/ic-ci":
                    self._send_json(factor_analysis.ic_confidence_interval(config.db_path, factor_col=qs.get("factor", ["mass_zscore"])[0], forward_days=int(qs.get("days", ["5"])[0])))
                elif path == "/api/ic-after-clip":
                    self._send_json(factor_analysis.ic_after_clip(config.db_path, factor_col=qs.get("factor", ["mass_zscore"])[0], forward_days=int(qs.get("days", ["5"])[0])))
                elif path == "/api/ic-boxplot":
                    self._send_json(factor_analysis.ic_boxplot(config.db_path, factor_col=qs.get("factor", ["mass_zscore"])[0]))
